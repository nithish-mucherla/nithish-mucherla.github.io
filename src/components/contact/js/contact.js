...
class Contact extends React.Component {
  ...
  handleSubmit(event) {
    ...
    if (this.state.isValid) {
      const templateId = "template_AoABT9JZ";
      this.sendFeedback(templateId, {
        message_html: this.state.message.value,
        from_name: this.state.name.value,
        from_mail: this.state.email.value,
      });
      this.setState({
        name: {
          error: "",
      ...
      this.setState({
        successMessage: "Hey! thanks for responding, have a nice day!",
      });
    }
  }
